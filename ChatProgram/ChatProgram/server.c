...
char *START_STRING = "Success to enter the chatting.\n"; // 클라이언트 환영 메시지
...
//Server Intro
void *thread_intro(void *arg) {
	int i;
	
	fprintf(stderr, "\033[1;33m");
	printf("@@@@CHAT SERVER IS RUNNING!!@@@@");
	
	printf("     NOW NUMBER OF USERS : %d\n", num_user);
	if(num_user == 0) {
		printf("Nobody chating now..\n");
	}
	else{
		printf("Chatting is occuring now!");
	}
...
int main(int argc, char *argv[]) {
	...
	char buf[MAX_LINE + 1]; //Reserved by Clients
	...
	//스레드 생성
	pthread_create(&thread_work, NULL, thread_intro, (void *)NULL);
	while (1) {
		FD_ZERO(&read_fds);
		...
		if (FD_ISSET(listen_sock, &read_fds)) {
			...
			printf("Someone come in!  All Users = %d\n", num_user);
		...
		for (i = 0; i < num_user; i++) {
			if (FD_ISSET(client_sock_list[i], &read_fds)) {
				...
				fprintf(stderr, "print the messages\n"); //커서 출력
...
void removeClient(int s) {
	...
	fprintf(stderr, "\033[33m");//글자색을 노란색으로 변경
	printf("[%02d:%02d:%02d]", tm.tm_hour, tm.tm_min, tm.tm_sec);
	printf("Someone out.  Users Count = %d\n", num_user);
	fprintf(stderr, "\033[32m");//글자색을 녹색으로 변경
	fprintf(stderr, "...Server is still alive..."); //커서 출력
}
...
int  tcp_listen(int host, int port, int backlog) {
	...
	if (bind(sd, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
		perror("fail to bind. Retry plz.");  exit(1);
	}
	// 클라이언트로부터 연결요청을 기다림
	listen(sd, backlog);
	return sd;
